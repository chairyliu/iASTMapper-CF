...
public class BestMappingSearcher {
    ...
    public void findElementPairsToMap(Map<ProgramElement, ProgramElement> elementsToMap,
        ...
        if (!excludeCurMappings) {//false代表全局，true代表局部
            Map<ProgramElement, Set<ProgramElement>> myLocal = new HashMap<>();//一个源对应一个目标集合，以map形式统一存储
    ...
    private void findBestDstCandidates(ProgramElement srcElement) {
        ...
        BestCandidateSearcher searcher = new BestCandidateSearcher(srcElement, candidateElements,
                eleMappings, determiner, candidateSearcher.getCandidateSetsAndMaps());//寻找最佳的目标元素候选，最佳的候选目标元素是根据相似度的最大值确定的
        Set<ProgramElement> globalBestDstCandidates = searcher.getBestGlobalCandidates();//获取全局最佳目标元素候选集合
        if (!eleMappings.isMapped(srcElement)) {//如果srcElement未被映射，获取局部最佳目标元素候选集合
            Set<ProgramElement> localBestDstCandidates = searcher.getBestLocalCandidates();
            if (localBestDstCandidates != null)
                srcToLocalBestDstCandidateMap.put(srcElement, localBestDstCandidates);//存储
        }
        //将全局最佳目标元素候选集合存储在srcToGlobalBestDstCandidateMap中
    ...
    //首先获取两个全局的候选项集合（源到目标，目标到源），1.如果目标元素到源只有一个候选项，源也只有一个到目标的候选项，则映射成功
    //如果目标元素有多个源候选项，调用BestCandidateSearcher
    private void findOneToOneBestElementPairsToMap(Map<ProgramElement, ProgramElement> elementsToMap,
        ...
        for (ProgramElement srcElement: srcToBestDstCandidateMap.keySet()) {//遍历源到最佳目标的候选项映射，得到srcElement集合
