...
//        this.filesToAnalyze = filesToAnalyze;//删，因为后面的pathMap可以获取到每个commitId下的所以file路径对，原来的一对一需要将pathMap筛选到只分析当前的文件，所以设置了这样一个参数
    ...
    public void calResultMapping(boolean doComparison, boolean doEvaluation) {//可以获取pathMap后将整个map传入RevisionAnalysis中
        ...
        for (String srcFilePath : pathMap.keySet()){
            i++;
//            if (commitId.equals("06bfecb6eddbc28bd971c4879ec7ef30108a6bf8") && srcFilePath.equals("activeio/src/java/org/activeio/adapter/SyncToAsyncChannel.java"))
...
//                    if (commitId.equals("06bfecb6eddbc28bd971c4879ec7ef30108a6bf8") && srcToPath.equals("activeio/src/java/org/activeio/adapter/SyncToAsyncChannel.java"))
//                        System.out.println(srcToPath);
