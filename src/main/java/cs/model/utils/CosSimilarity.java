...
public class CosSimilarity {
    ...
    public Double cosineSimilarity(Map<CharSequence, Integer> leftVector, Map<CharSequence, Integer> rightVector) {
        if (leftVector != null && rightVector != null) {
            ...
            Integer value;
            for(Iterator var8 = leftVector.values().iterator(); var8.hasNext(); d1 += Math.pow((double)value, 2.0)) {
...
//            Integer value;
            for(Iterator var10 = rightVector.values().iterator(); var10.hasNext(); d2 += Math.pow((double)value, 2.0)) {
            ...
            if (!(d1 <= 0.0) && !(d2 <= 0.0)) {
                cosineSimilarityB = dotProductB.divide((d1B.sqrt(new MathContext(5, RoundingMode.HALF_UP)).multiply(d2B.sqrt(new MathContext(5, RoundingMode.HALF_UP)))), 5, BigDecimal.ROUND_HALF_UP);
            } else {
